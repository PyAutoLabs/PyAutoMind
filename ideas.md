...
- [from: research jax-autodiff-gradients-audit (#87) · arXiv:2606.30620] Differentiable adaptive rectangular mesh: replace the empirical point-rank CDF in `create_transforms` with a smooth-density CDF (Enzi et al. RTU formulation) so `RectangularAdaptDensity` carries smooth mass/shear gradients — today at os_pix=1 the likelihood is exactly piecewise-constant in mass params (staircase; gradient-based mass inference impossible), and os_pix>1 only partially restores signal.
