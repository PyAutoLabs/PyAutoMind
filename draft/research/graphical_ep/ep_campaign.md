...
1. An analytic Gaussian model that statistically demonstrates the graphical
   ...
   `analytic_gaussian` (ledger `projects/analytic_gaussian.md`),
...
| # | Phase | Prompt | State (updated 2026-09-10) |
|---|-------|--------|--------------------|
...
| 1b | Confidence test on the #1405 collapse toy at N=50 (NUTS vs EP x3) | → PyAutoCortex `projects/ep_toy_gaussian.md` | **born 2026-09-10** as Cortex science project `ep_toy_gaussian` (Cortex PR #29, human merge pending — `projects.yaml` is code). The campaign's last confidence test before scaling up: the *original* collapse toy from `complete/2026/07/ep_scale_collapse_assets/ep_toy_diagnostic.py`, model and EP settings verbatim (`DynestyStatic(nlive=100, rwalk)` per factor, `LaplaceOptimiser`, `EPHistory(kl_tol=0.05)`, `max_steps=20`), but at **N=50**, with a joint **BlackJAX NUTS** reference and **three EP repeats on identical data** — none of which this toy has ever had. A pass on the no-collapse / parity / self-agreement / honest-width criteria is the evidence that the D1–D6 + review-wave fixes cured PyAutoFit#1405 on its own toy; a failure isolates it to the Dynesty-per-factor EP path, since the analytic ensemble used Laplace factors. Task `State: ready`, witness pre-registered (7 criteria), **awaiting submission** |
...
PyAutoCortex `archive/tasks/slope_hierarchy/methods_writeup.md`, never run — it stayed
...
- **JAX/gradient/Hessian EP internals** — against changing EP internals while
  ...
  EP update maths itself.) — 2026-09-02: the moment-matching cure prompt is the first candidate for this gate. 2026-09-07: still the human's call (`Autonomy: supervised`); the phase-2 review bundle did not touch the update maths. 2026-09-24: 343299 shows the Laplace hierarchical projection fails on every update at N=25 lensing; the moment-matching cure is now the blocking decision for phase 3's EP arm; ep_toy_gaussian wave 1 failed on infra (EMFILE, fixed by #1632/#1634), wave 2 awaits a human go.
