...
| 2 | The same profiling on CPU (CPU-appropriate methods) and on the laptop RTX 2060 | `active/fixed_light_cpu_and_consumer_gpu.md` (autolens_profiling #253) | phase 1 library-path rows measured | **issued #253** 2026-09-13 — worktree `fixed-light-hardware`, branch `feature/fixed-light-hardware` stacked on phase 1's |
