...
Blocked on 2026-09-25: autolens_profiling claimed by `certified-solver-phase-c1-lane-rate`; euclid_strong_lens_modeling_pipeline claimed by `vis-lp-inspection-bundle` (`euclid-dr1-positions-gate`/`-finder` closed 2026-09-25). Apply phase 1's exact block once those merge. autolens_profiling's `activate.sh` sources the shared venv directly (no `.venv` branch) — put the block after that `source`.
