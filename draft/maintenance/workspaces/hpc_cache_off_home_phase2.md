...
Blocked on 2026-09-25: autolens_profiling claimed by `certified-solver-phase-c1-lane-rate`; euclid_strong_lens_modeling_pipeline claimed by `euclid-dr1-positions-gate`, `euclid-dr1-positions-finder`, `vis-lp-inspection-bundle`. Apply phase 1's exact block once those merge. autolens_profiling's `activate.sh` sources the shared venv directly (no `.venv` branch) — put the block after that `source`.

Original request and background: see phase 1 prompt.
