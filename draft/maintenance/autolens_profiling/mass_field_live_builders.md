...
- autolens_profiling
Epic: mass-field
Status: draft — awaiting recorded parallel-claim waiver
Autonomy: supervised
Filed: 2026-09-19
Difficulty: large
...
Run AST audit, Ruff, README generator check, PR smoke, and one representative imaging/interferometer/point-source script where available. Do not rewrite committed result rows from past versions. The repo is currently claimed by `hst-gpu-residue-p2`; its dirty files are the excluded `fixed_light_trace.py`, an HPC submit script and result artifacts. A separate worktree needs a human waiver recorded in `active.md` before source edits.
