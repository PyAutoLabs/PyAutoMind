...
| 8 | active/user_workspace_howto_slow_script_pass.md | **PR OPEN** 2026-09-06 — autolens_workspace#536; HowToLens#77 (3 chapter-3 meshes capped with the data: 14.0/10.0/7.7 s → 4.5/4.2/4.1 s, suite 201 → 182 s) and HowToGalaxy#73 (11.9 → 4.6 s) awaiting merge; the local-vs-CI gap was a cold numba cache + `should_simulate` re-simulation, not the scripts; autolens_workspace / autogalaxy_workspace unchanged — their findings are library work, filed as phase 8c: `draft/bug/autofit/replace_promise_no_op_graph_walk.md`, `draft/bug/autoarray/small_datasets_cap_stamp_stops_resimulation.md`, `draft/bug/autoarray/sparse_operator_ignores_disable_jax.md`, `draft/feature/autoarray/mesh_shape_honours_small_datasets_cap.md`; 8b `draft/test/autocti_workspace/imaging_ci_start_here_61s.md` |
