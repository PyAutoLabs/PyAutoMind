...
| 8c | active/{replace_promise_no_op_graph_walk,small_datasets_cap_stamp_stops_resimulation,sparse_operator_ignores_disable_jax,mesh_shape_honours_small_datasets_cap}.md | **PRs OPEN** 2026-09-06 — PyAutoNerves#160 → PyAutoArray#529 (merge in that order) and PyAutoFit#1564, awaiting merge; measured: re-simulation gone on the three interferometer/multi scripts (12.4/10.5/10.9 s → 6.7/6.3/6.5 s), shapelets `model.info` 10.6 → 6.7 s, both `_test` gates green with the branches installed. Issued as the library leg of phase 8: PyAutoFit#1563 (`replace_promise` no-op walk, 10.1 → 5.4 s on the shapelets script), PyAutoNerves#159 (`disable_jax()` predicate + `SMALLSHP` cap card), PyAutoArray#528 (stop re-simulating capped interferometer/multi datasets ~26–28 s per CI run; `apply_sparse_operator` honours `PYAUTO_DISABLE_JAX`; mesh `shape=` capped with the data); library-first Nerves → Array, Fit independent |
| 9 | active/unit_test_import_time_hotspot_census.md | **PR OPEN** 2026-09-06 — PyAutoHeart#213 → #214 awaiting merge: seven findings (library CI is compile-bound because `lib-tests.yml` restores no JAX/numba cache; `--cov` is a third of the legs; one kaplinghat test is 52 % of PyAutoGalaxy; three imports paid and not needed; PyAutoFit unparallelisable; the NUFFT "numpy" path is JAX; fixtures are not a finding), nine ranked options, O1 = caches for `lib-tests.yml` (the census: unit-test rows from the first `unit-timings` artifacts + local `--durations`, import trees, shared hot spots, ranked options; deliverable `timings/unit_import_census_2026-09.md`) |
