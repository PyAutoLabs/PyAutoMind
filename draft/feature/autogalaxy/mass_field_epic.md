...
| 9 | **PR ready 2026-09-19** — `active/mass_field_flat_adoption_developer.md` | autolens_workspace_developer | Live JAX, search, plotting, LOS and standalone MGL builders migrated; measurement archives preserved. | autolens_workspace_developer#143 consumes the helper from current source `main`; point-source pin drift and legacy examples are recorded on issue #142. |
...
- 2026-09-18: **phase 6's sweep completed** — autolens_workspace#562 and autolens_workspace_test#322, both DRAFT pending the PyAutoGalaxy/PyAutoLens PyPI release. Its follow-up audit filed five drafts: the chaining-helper task (now `complete/2026/09/mass-field-chaining-helper.md`; a chained stage had silently dropped the field in `multi_dataset/features/one_by_one/modeling.py`, fixed in #562); `draft/maintenance/autolens_profiling/mass_field_flat_adoption_science_repos.md` (original four-repo survey, now split); the developer workspace sweep (now `active/mass_field_flat_adoption_developer.md`); `draft/maintenance/pyautomind/autolens_jax_joss_manifest_gap.md`; and `draft/test/autolens_workspace_test/latent_smoke_assertion_echoes_search_log.md`. **Phase 5 was amended the same day** so HowToLens migrates once, straight to the flat form, rather than twice.
