...
Priority: normal
Consequence: judge
Witness: in both repos `grep -rnE "al\.Galaxy\(.*(shear|mass_sheet|potential)=" scripts/` and `grep -rn "shear_galaxy" scripts/` return nothing except the one legacy regression script `autolens_workspace_test/scripts/misc/mass/galaxy_attached_legacy.py` and the legacy case inside `misc/interop/coolest_round_trip.py`; no `MassField` appears inside a `galaxies` collection or list; the traced grid at fixed instance values of the migrated `imaging/modeling.py`, `multi_galaxy/modeling.py` and `group/modeling.py` models is `np.allclose` to the pre-change one; the local smoke subset (listed on the issue) is green against library `main`; both repos' CI smoke green; notebooks, `workspace_index.json` and `llms-full.txt` regenerated; `check_navigator.py --root <checkout> --banners=fail` clean from the parent directory.
Review-minutes: 15
