...
<!-- generated by `pyauto-brain intake dashboard --apply` on 2026-09-08 — regenerate, do not hand-edit -->
...
> **Last updated 2026-09-08.** This page is generated from `active/`, `draft/` and the registry files, so it is only as current as they are. `dashboard_refresh.yml` re-renders it on every push to `main` — that heals a stale page, but not a stale prompt: a task that shipped without its prompt advancing to `complete/` keeps rendering here as pickable backlog. Reconciling those is the refresh below.
...
> - `draft/feature/autoarray/sibson_single_concatenated_walk.md — its own `Status:` says superseded, but the prompt never left draft/ (reconcile its lifecycle)`
...
```
/start_dev draft/bug/autonerves/xla_gpu_autotune_level_0_default_slows_fp64_gemm.md
```
...
<details><summary>📋 <a href="active/batch_notify_tier_merge.md">The tier-<code>notify</code> auto-merge decision — re-open the shadow window, then decide</a> — <a href="https://github.com/PyAutoLabs/PyAutoBrain/issues/364">issue #364</a> — issued 2026-09-07 — PR open — PRs: <a href="https://github.com/PyAutoLabs/PyAutoBrain/pull/366">PyAutoBrain#366</a>, <a href="https://github.com/PyAutoLabs/PyAutoMind/pull/395">PyAutoMind#395</a></summary>
...
<details><summary>📋 <a href="active/numba_interferometer_kernel_levers.md">Numba vs JAX-CPU interferometer curvature: synthetic bake-off, kernel levers, and the reinstatement…</a> — issued 2026-09-07</summary>

```
/start_dev active/numba_interferometer_kernel_levers.md
```
...
<details><summary>📋 <a href="active/samples_errors_at_sigma_instance_prior_valued_model.md"><code>Samples.errors_at_sigma(as_instance=True)</code> crashes on a model whose component class is a Prior</a> — issued 2026-09-07</summary>

```
/start_dev active/samples_errors_at_sigma_instance_prior_valued_model.md
```
...
<details><summary>📋 <a href="draft/feature/autoarray/sibson_single_concatenated_walk.md">Sibson natural-neighbour: one concatenated Delaunay locate instead of two</a> — autoarray · small · safe · medium</summary>

```
/start_dev draft/feature/autoarray/sibson_single_concatenated_walk.md
```
...
<details><summary>📋 <a href="draft/bug/autoarray/mapper_adapt_zero_signal_jax_nan.md">Adapt-density mapper: a zero-signal adapt image is NaN on the JAX path…</a> — autoarray · small · safe · medium</summary>

```
/start_dev draft/bug/autoarray/mapper_adapt_zero_signal_jax_nan.md
```
...
- draft/feature/pyautobrain/batch_slice.md
...
| Prompt | Repo | Difficulty | Priority | Status |
|--------|------|------------|----------|--------|
| <a href="draft/feature/pyautomind/witness_campaign.md">Witness campaign — make the backlog reviewable by construction</a> | pyautomind | medium | high | draft |
| <a href="draft/feature/pyautobrain/batch_slice.md"><code>batch slice</code> — the decomposition pass for <code>needs-slicing</code> prompts</a> | pyautobrain | medium | normal | draft |
...
<details>
<summary><b>jax-gpu · performance</b> — 2 task(s) · 2 pts · auto — proposed — ⚠️ theme(s) not in `themes.md`: delaunay, hpc, jax-gpu, performance</summary>

<details><summary>📋 <b>Run this bundle</b> — one session, one issue and one PR per member</summary>

```
You are the architect (Fable) for the PyAutoMind bundle 'jax-gpu · performance' — 2 INDEPENDENT tasks run in one orchestrated session.
...
<details>
<summary><b>mge</b> — 2 task(s) · 2 pts · auto — proposed</summary>

<details><summary>📋 <b>Run this bundle</b> — one session, one issue and one PR per member</summary>

```
You are the architect (Fable) for the PyAutoMind bundle 'mge' — 2 INDEPENDENT tasks run in one orchestrated session.

Members:
- draft/test/autolens_workspace_developer/mge_jit_regression_rebaseline.md
...
| Prompt | Repo | Difficulty | Priority | Status |
|--------|------|------------|----------|--------|
| <a href="draft/test/autolens_workspace_developer/mge_jit_regression_rebaseline.md">Re-baseline the MGE imaging JIT profiling regression value</a> | autolens_workspace_developer | small | high | formalised |
| <a href="draft/docs/autolens_workspace/markdown_regeneration_sigma_min.md">Regenerate autolens_workspace markdown/ so the MGE pages show…</a> | autolens_workspace | small | normal | formalised |
...
| 2026-09-07 | issued | <a href="active/batch_notify_tier_merge.md">The tier-<code>notify</code> auto-merge decision — re-open the shadow window…</a> |
...
```
Continue the 'Numba CPU interferometer likelihood revisit — recover, break down, judge reinstatement' epic. Its canonical state lives in draft/research/autolens_profiling/numba_interferometer_likelihood_revisit.md — read that ledger (and any DECISIONS/RESULTS files beside it) first. Cross-check this epic's entry in PyAutoMind/epics.md, any related rows in PyAutoMind/active.md, and the referenced repos' open issues and PRs, to work out the last completed phase and what is currently in flight. Then pick the next logical step and continue it through the normal workflow (/start_dev — filing the phase's prompt first if none exists), updating the ledger as the work advances. Note: 3 phased prompts (feature pack → research levers/verdict → research preload); issued one at a time, phase 1 first; PyAutoArray read-only throughout, library follow-ups filed via /intake from the verdict.
```

</details>

<details><summary>📋 <a href="draft/research/autolens_profiling/interferometer_preload_cpu.md">The interferometer preload on CPU: builder timings and the adjoint-NUFFT construction</a> — autolens_profiling · medium · supervised · high</summary>
...
<details><summary>📋 <a href="draft/research/autolens_profiling/numba_interferometer_likelihood_revisit.md">Revisit the numba CPU interferometer likelihood — epic ledger</a> — autolens_profiling · too-large · human-required · high</summary>

```
/start_dev draft/research/autolens_profiling/numba_interferometer_likelihood_revisit.md
```
...
```
Continue the 'Expectation propagation (EP) campaign' epic. Its canonical state lives in draft/research/graphical_ep/ep_campaign.md — read that ledger (and any DECISIONS/RESULTS files beside it) first. Cross-check this epic's entry in PyAutoMind/epics.md, any related rows in PyAutoMind/active.md, and the referenced repos' open issues and PRs, to work out the last completed phase and what is currently in flight. Then pick the next logical step and continue it through the normal workflow (/start_dev — filing the phase's prompt first if none exists), updating the ledger as the work advances. Note: umbrella phase map — each phase's real content lives in its own prompt under draft/research/graphical_ep/; the campaign file itself is never issued. Science half: PyAutoCortex epics.md#graphical-ep (campaign phases 3 and 4).
```
...
- `draft/feature/autoarray/sibson_single_concatenated_walk.md — unknown theme keyword(s): jax-gpu, delaunay, performance`
...
- `draft/bug/autonerves/xla_gpu_autotune_level_0_default_slows_fp64_gemm.md — unknown theme keyword(s): jax-gpu, performance, hpc`
...
- `draft/research/autolens_profiling/interferometer_preload_cpu.md — unknown theme keyword(s): nufft`
