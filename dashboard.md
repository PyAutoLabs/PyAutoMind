...
<!-- generated by `pyauto-brain intake dashboard --apply` on 2026-09-05 — regenerate, do not hand-edit -->
...
> **Last updated 2026-09-05.** This page is generated from `active/`, `draft/` and the registry files, so it is only as current as they are. `dashboard_refresh.yml` re-renders it on every push to `main` — that heals a stale page, but not a stale prompt: a task that shipped without its prompt advancing to `complete/` keeps rendering here as pickable backlog. Reconciling those is the refresh below.
...
**155** filed prompts, not started. Each section is sorted most-pickable first (priority, then size). **32** of them belong to an epic and are listed only under [Epics](#epics) below.
...
| 2026-09-05 | issued | <a href="active/legacy_baseline_timing_round.md">Legacy baseline timing round: snapshot pre-rebuild timings of every…</a> |
...
<details>
<summary><b>CI test timing — finish the board, fast physical tests</b> — 6 queued prompt(s), in order</summary>
