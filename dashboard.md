...
**208** filed prompts, not started. Each section is sorted most-pickable first (priority, then size). **37** of them belong to an epic and are listed only under [Epics](#epics) below.
...
```
You are the architect (Fable) for the PyAutoMind bundle 'docs-hub' — 4 INDEPENDENT tasks run in one orchestrated session.
