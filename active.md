...
- next: rigorous drizzle-consistency (route a vs frame_combine b, sub-pixel phase) on a genuinely STELLAR field, THEN wire build_starred_epsf (still hard-stop). Traps: reduction.json "96 stars" = JWST-tuned config not strict point-src (defaults find ~2); worktree lacks gitignored outputs → spike reads canonical checkout output; STARRED needs starred venv (astropy8/jax0.10 ≠ PyAuto).
