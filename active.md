...
- repos:
  ...
  - PyAutoFit: feature/catalogue-mass-maps-fits
- library-pr: https://github.com/PyAutoLabs/PyAutoFit/pull/1633
- workspace-pr: https://github.com/PyAutoLabs/euclid_strong_lens_modeling_pipeline/pull/81
...
- note: "Plan approved 2026-09-16, parked, then STARTED the same day via /start_workspace (Fable architect session, Opus execution). Full two-level plan on the issue. Key reframing: this is a collect, not a compute — every finished initial_lens_model search already writes image/tracer.fits (MASK, CONVERGENCE, POTENTIAL, DEFLECTIONS_Y, DEFLECTIONS_X on the zoomed mask grid +1 px, 102x102 for sep1) into its result zip, and al.agg.fits_tracer + af.AggregateFITS.extract_fits read it out read-only, exactly as catalogue/scripts/deblending.py does for model.fits. New producer catalogue/scripts/lens_mass_maps.py (vis_pix only, three files), new bundle stage, hpc/sync sbatch-arg forwarding + pull [dir...] filter, PROJECT_PATH from submit location. Witness: sha256 of all 18 sep1 zips identical before/after a local run through a scratch symlink; 9 lenses x 3 files, Tile102007299 skipped. Science checkout /mnt/c/Users/Jammy/Science/euclid_dr1 is not a Cortex projects.yaml row (flagged, not fixed here). SHIPPED 2026-09-16: PR https://github.com/PyAutoLabs/euclid_strong_lens_modeling_pipeline/pull/81 @ 3a200ee, awaiting /prm; upstream defect filed draft/bug/autofit/aggregate_fits_tracer_json_shadows_tracer_fits.md; after merge the human runs the science-side recipe in Science/euclid_dr1 (git merge origin/main; hpc/sync push --no-data; hpc/sync submit cpu submit_build_inspection_bundle --export=ALL,SAMPLE=dr1_sep1; hpc/sync pull inspect). PyAutoFit fix shipped 2026-09-16 (https://github.com/PyAutoLabs/PyAutoFit/pull/1633): AggregateFITS resolves FITS sources through result.fits; pipeline follow-up commit 13876cb deletes the AggregateTracerFITS stand-in; library-first — merge the PyAutoFit PR before #81; the PyAutoFit worktree lives in the same task worktree. RAL runs library mains via HPCPullPyAuto, so no PyPI release-gate is recorded."
