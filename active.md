...
- status: PHASE 1 BORN, publish gate 2/3 — PyAutoLabs/autocti_assistant EXISTS (3a7a382, PRIVATE, 151 files, 388 PENDING), registered in repos.yaml. Ph0 merged (PyAutoBrain#137 8db0c8e + autolens_assistant#76 dfa07b1). HEART PUBLISH GATE: leg1 symbol audit PASS (10/10), leg2 link sweep PASS (0 dangling/37 files), LEG3 wiki-currency FAIL = BLOCKS PUBLIC, leg4 chat smoke deferred (needs public URL). LEG3 ROOT CAUSE (inherited content, NOT a seed bug): copied wiki-currency.yml installs the RELEASED stack from PyPI (autolens shape) — for CTI that gives (a) no arcticpy (hard import of autocti, NOT a pip dep; needs libgsl-dev+cython, downgrades numpy <2.0; known-good recipe = autocti_workspace_test/.github/scripts/smoke_install.sh), (b) STALE 2024.11.13.2 PyPI wheel because the CTI release train is unwired (draft/release/autocti/cti_release_train_wiring.md, human-required) — grading modern docs against it would pass VACUOUSLY, and leg3's OWN spec says 'sources clone @ main, NOT the release wheel', (c) expects autogalaxy which is not a CTI dep. Install exits 3 so checks never run. NEXT DECISION (before Ph2): adapt the newborn's wiki-currency to source-install + arcticpy, vs wait on the CTI release train. BOUNDARY FINDING for the reference: '.github/*' blanket-generic is TOO COARSE — a workflow's install step is domain content (same shape as Ph0's 39, one layer deeper, only surfaceable by a real birth). PYTHONPATH TRAP: ambient PYTHONPATH has the editable checkouts but OMITS PyAutoCTI — APPEND it, never replace, or autoarray/autofit silently drop to old site-packages (2026.4.5.3) and the gate passes vacuously
- autonomy: supervised
