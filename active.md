...
- next: DEFINITIVE comparison NEEDS a genuinely stellar well-sampled field = dedicated reduction (globular-cluster/stellar-cal ACS field, or lens field w/ foreground stars) — HEAVY (MAST+drizzlepac+CRDS), user-decision pending. Back-end functionally COMPLETE (ground-truth consistency + e2e + 206 tests); could ship now pending the stellar-field confirm, OR do the reduction first. + workspace example exposing psf_backend="starred". Traps: reduction.json "96 stars"=JWST-config not strict point-src; worktree lacks gitignored outputs→reads canonical checkout; build_starred_epsf runs only in starred venv w/ autoreduce on PYTHONPATH.
