...
- status: awaiting-merge — ALL 12 PRs OPEN, none merged (RED override covers PR-open only). autolens_workspace#318, autogalaxy_workspace#146, autolens_workspace_test#200, autogalaxy_workspace_test#81, HowToLens#45, HowToGalaxy#36, autolens_assistant#87, autocti_assistant#8, euclid_strong_lens_modeling_pipeline#32, PyAutoCTI#97, PyAutoArray#401, PyAutoGalaxy#519 — all labelled pending-release. RESUME: merge all 12 once Heart clears, then `lifecycle.py record` + drop this entry. Was: 13 pure file deletions (10 orphan `config/grids.yaml` + 2 library `config/README.md` bullets + 1 `autocti_assistant/PENDING.md` TODO). Nothing has read `radial_minimum` since PyAutoGalaxy 025ac7ac (2025-10-15); zero `.py` hits across all five libraries and zero hits in the installed venv. Landing as 3 PRs: (1) PyAutoCTI test config, (2) PyAutoArray+PyAutoGalaxy READMEs, (3) the 8 downstream repos.
...
- override: Heart RED (verdict `red`, score 0) commit/push/PR-open block OVERRIDDEN by direct contemporaneous human authorization 2026-07-23, sought against the FULL seven-reason set verbatim (an earlier ask quoted only one reason and was re-sought): `PyAutoFit: 1 commit(s) behind origin` / `PyAutoGalaxy: 1 commit(s) behind origin` / `PyAutoLens: 1 commit(s) behind origin` / `release validation FAILED (stage integrate)` / `workspace validation not passing (13 failed, 2026-07-21T19-05-22Z)` / `33 stale parked script(s)` / `manifest drift: tenant firewall (organ code) — 2 mismatch(es) vs PyAutoMind/repos.yaml`. This is NOT the AUTONOMY.md corrective-PR exception — the diff repairs none of the seven. Task-specific, does NOT extend to merge, release stays blocked until each reason clears on its own terms. Recorded on issue #317 (comment 5057485854). LESSON: read the full reason list from `pyauto-heart readiness --json`, not the vitals summary, before asking a human to acknowledge RED.
