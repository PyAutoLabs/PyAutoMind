...
- status: workspace-dev — SLOWDOWN CLAIM RETRACTED: venv activate:74 exports XLA_FLAGS=--xla_disable_hlo_passes=constant_folding globally = 3.2x tax (mge 0.946s->0.292s control); imaging cells RERUNNING flag-free + runtime mge with/without control pair for #56 (phase-3 local-CPU matrix likely inflated, A100 rows honest = ratios skewed; cross-posted #56); provenance now in JSONs (7edf06a); venv-flag removal batched on #59; alma_high cells = GPU-only tier (2x 2h NUFFT timeouts, 3rd killed-as-certain); A100 leg dispatch-ready (15 submits) but RAL GPU nodes DOWN
...
- status-update 2026-07-10: PR-OPEN awaiting human merge — library scoped-preloads follow-up PyAutoArray#381 (first) + PyAutoLens#601 (cross-type hazard found in review: lead preloads reach every factor; fits now scope consumption to mesh view cross-type); workspace #261 (docstring sections) + workspace_test #162 (parity: bit-identical + g+r jit) + profiling #60 (1.14x at hst x 4 — honest mesh-only ratio). Gate record on issue #260. Heart YELLOW set verified identical to in-session ack.
