...
- next: wire build_starred_epsf (currently hard-stop) — MANDATE: centroid-preserving delivery not naive crop; add MIN_STARS/SNR + sampling-threshold guards; drizzle drop-shape (pixfrac<1) = 2nd-order refinement to test on real data. Traps: reduction.json "96 stars"=JWST-config not strict point-src (defaults find ~2); worktree lacks gitignored outputs→spike reads canonical checkout; STARRED needs starred venv (astropy8/jax0.10≠PyAuto).
