...
- workspace-pr: https://github.com/PyAutoLabs/HowToFit/pull/54
- workspace-pr: https://github.com/PyAutoLabs/autofit_workspace/pull/155
- status: pr-open (both PRs open 2026-09-14; CI pending, not merged)
- was-blocked: "RESOLVED 2026-09-14 by the human running the pushes by hand. Both repos COMMITTED locally, trees clean, nothing left to author: HowToFit 8fbff31 (40 files), autofit_workspace 4adf61e (8 files). Blocked at PUSH by the Claude Code auto-mode Bash classifier, which denies `git push` of feature/howtofit-colab-and-chapter-advanced in both repos with reason [Auto-Mode Bypass] (a subagent hit the same denial as [CI Bypass]). This is a HARNESS permission layer, entirely separate from the Heart RED gate the human already overrode — pushes to PyAutoMind main from the same session succeed, so it is specific to these feature branches. Not routed around via gh api ref-creation: that would defeat the control's intent. Unblock = the human adds a Bash permission rule, or runs `git push origin feature/howtofit-colab-and-chapter-advanced` in both worktrees themselves. Then PR-open + /prm can resume with nothing re-authored."
- heart-override: "2026-09-14. Shipped with pyauto-heart readiness RED, reason verbatim 'release validation FAILED (stage integrate)' (stale_reasons: [], not staleness). This is an EXPLICIT LIVE HUMAN OVERRIDE — the user, after being shown the RED reason and the policy, said 'actually just get this all live in spite of the red'. It is NOT the AUTONOMY.md corrective-PR exception, which does not apply here: that permits only a PR repairing the exact defect the RED names, and nothing in this task touches it (the RED is autolens/autolens_test workspace validation, manifest drift vs repos.yaml, profiling drift). Recorded so the override is auditable and is never mistaken for policy-sanctioned precedent. Merge/close remain human via /prm."
- summary: |
