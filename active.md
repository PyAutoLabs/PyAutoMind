...
- summary: |
    Clears the Heart profiling-drift YELLOW (6 drifted of 123 scanned), neither
    cause being real drift. (A) The three results/runtime/imaging MGE summaries
    are pre-re-pin discovery JSONs committed by 446f8f1 (#235): each got already
    equals today's pin, so they are regenerated by re-running the cells locally.
    (B) matrix_free.py appends unconditional SLQ (p=16 m=40) comparison records
    with rtol:null into pinned_drift ("recorded, never a fault"), but Heart's
    contract is "empty pinned_drift = all matched", so every one reads as drift.
