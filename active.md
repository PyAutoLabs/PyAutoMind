...
- status: awaiting-merge
- pr: https://github.com/PyAutoLabs/autolens_profiling/pull/275
- library-pr: https://github.com/PyAutoLabs/PyAutoArray/pull/557
- hpc: "CPU job 343394 (fl_numba_s4) COMPLETED 0:0 in 00:02:42 (submitted 15:37 BST; HARVESTED as 5c8ca44, fetched over ssh — RAL has no GitHub push credential) from RAL worktree /mnt/ral/jnightin/autolens_profiling_wt/fixed-light-numba-s4 @ c7b72f6 (gpu partition CPUs-only, euclid-ral-gpu-1, one 1-CPU neighbour 343391_0 at start; lensed_source.fits pre-seeded from the levers worktree, md5 d15c4ce9); routes b,b_direct,b_touched --n-repeats 64 then the s4 witness; WALL-BASIS said 7200 s, measured 162 s (re-pin is a Next item). Harvest done: commit results/breakdown/imaging/fixed_light_numba_delaunay_hpc_ral_cpu_fp64_fixed_light_numba_s4_b_warm_t1.{json,png} + fixed_light_numba_s4_witness_hpc_ral_cpu_fp64_s4.json in the RAL worktree, fetch locally, read rows.*.call_ms on all three rows with ABBA + overhead PASS and seed_source memo, apply the >= 5 % whole-call rule; output/ error/ logs in hpc/batch_cpu/ of the RAL worktree."
- worktree: ~/Code/PyAutoLabs-wt/fixed-light-numba-s4
- repos:
  - autolens_profiling: feature/fixed-light-numba-s4
  - PyAutoArray: feature/fixed-light-numba-s4
...
- note: "Phase 4 of fixed-lens-light-numba-cpu. Two waves, sequential PRs, never stacked (#267 lesson). WAVE A SHIPPED TO PR-OPEN 2026-09-17 16:15: autolens_profiling PR #275 (head 7fcb3f4, pending-release, lint GREEN) and PyAutoArray PR #557 (docstring-only at inversion_imaging_numba_util.py:781, 40a72b3e, unit tests pending at ship) — independent, either merge order. VERDICT (RAL job 343394, rule >= 5 % whole-call + ABBA PASS): NO LEVER — b 230.149 ms, b_direct 244.924 (+6.42 %), b_touched 236.913 (+2.94 %), all ABBA PASS; witness PASS (W2 direct 4.1e-13, touched bit-identical; W3 7.4e-16; W4 85.0 / 102.1 / 97.0 ms); two-stage is right on Delaunay too at u0 1.604, docstring's Delaunay prediction refuted; control reproduces lever 3's 230.031 at +0.05 %; chain unchanged 413.301 -> 230.031. Note results/notes/fixed_lens_light_s4_2026_09.md. Heart RED install-verify/integrate acked for PR-open only. NEXT: human /prm on #275 and #557 (close-out removes the task worktree incl. the PyAutoArray leg and the RAL worktree /mnt/ral/jnightin/autolens_profiling_wt/fixed-light-numba-s4); then WAVE B (lever 4b A-prime: witness definition first, issue plan steps 9-12) as a new sequential PR off main after #275 merges. Shipped comment on the issue 2026-09-17 16:14."
...
- workspace-pr: https://github.com/PyAutoLabs/euclid_strong_lens_modeling_pipeline/pull/86
...
- resume: "2026-09-17 15:15 BST. Branch feature/witt-wynne-catalogue is LOCAL ONLY at e86f34d (6+ commits on origin/main 53f3c23: util, producer, hardening, merge of #85, stage renumber 7/10, docstring tidy); 196 fast tests green; nothing pushed because pyauto-heart is RED ('install verification FAILED (testpypi; checks F)', 'release validation FAILED (stage integrate)') — push/PR need the human's explicit authorisation. Deliverable 5 (Q1 end-to-end) IN FLIGHT ON RAL: the local vis_pix fit died with the previous session after 3 h (no checkpoint); resubmitted as GPU job 343391 (euclid-ral-gpu-1, started 15:12 BST, expected 1h15-1h45) from the RAL mirror /mnt/ral/jnightin/euclid_strong_lens_modeling_pipeline (rsynced from clean main 53f3c23; fit code unchanged by the branch). HARVEST: from the worktree run `bash hpc/sync jobs`/`tail gpu`, then `bash hpc/sync pull output` (copy hpc/sync.conf from the canonical checkout first — it is gitignored and absent in the worktree), then `python catalogue/scripts/witt_wynne.py --sample=q1_walsmley`, then `SKIP_SED=1 CREATE_ARCHIVE=0 bash scripts/build_inspection_bundle.sh q1_walsmley`, PointSolver cross-check of the verdict, and fill the [Q1_RUNTIME]/[Q1_MACHINE]/[Q1_RESULT_SUMMARY] placeholders in the email draft (scratchpad email_draft.md; PR bodies pr_body_pipeline.md / pr_body_workspace.md drafted there too). Deliverable 6 (science clone run + Cortex ledger line) waits for the merge per the plan; scratch run on the ten DR1 tiles already gave 10/10 rows, PointSolver 9/10. Deliverable 7 email drafted."
...
- workspace-pr: https://github.com/PyAutoLabs/autolens_workspace/pull/553
