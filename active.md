...
- resume: MULTI-ROOT-CAUSE (not uniform jax-drift — refined #72 comment-4966797130). A=jax-drift FIXED (d1d3cfe). C viz_jit×3 = NOT config-gap (env_vars_release.yaml correct, TEST_MODE=0 real search) — fails on wheels 'no fit.png quick-update viz did not fire'; needs slow real-search release-env repro, likely timing/flake. B jax_grad×2 = autodiff-vs-FD + eager-vs-jit(rtol1e-10) gradient-audit asserts; repro VERY slow (FD >12min, didn't finish) — tolerance OR real gradient change TBD. D-H (database×6/aggregator×2/chaining/slam/interferometer/minimal_output) unclassified, maybe real bugs. I timeouts×5 perf. Best run as register_and_iterate cluster-by-cluster (several need slow real-search/FD repros), NOT one marathon. Inventory: active/release_validation_tail_burndown_2026_07.md. 3 blocker bugs DONE—don't reopen.
