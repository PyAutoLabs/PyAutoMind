...
- status: PHASE 3 MERGED (autocti_assistant#3, f061651) — wiki-currency GREEN in CI, legs 1-3 stay green — PyAutoLabs/autocti_assistant PRIVATE. Ph0-2 MERGED (skills live on main). PHASE 3 = wiki/core (index + 6 concept pages: CTI, trap physics, FPR/EPER, arctic algorithm, calibration strategy, parallel vs serial) + wiki/literature (PUBLIC-sources corpus, 8-paper bib VERIFIED via WebSearch against arXiv/ADS — Massey2010/2013/2014, AndersonBedin2010, Israel2015, Short2013, Skottfelt2017, MasseySM4; 6 source pages + 3 entities arctic/euclid_vis/hst_acs + schema) + wiki/project/state.md (resurrection state, release-train UNWIRED, legacy heritage). PRIVACY SEAM HELD: consulted PyAutoMemory wiki/cti for STRUCTURE/pointers only, wrote all content from public knowledge + web-verified cites, NEVER copied. De-lensed 2 generic seed leaks (wiki/README listed nonexistent euclid/ sub-wiki; wiki/project/README lensing slugs + dangling profile.md link). VALIDATION all green: --scope all, --check-citations, --lint-idioms, --check-provenance, --check-version + wiki link sweep 0 dangling. TRAPS: (a) WebSearch CORRECTED my paper-metadata memory 3x (Short=1302.1416 not 1301.x, Israel=1506.07831) — ALWAYS verify citations, never trust memory; (b) --check-citations does NOT check markdown links, only code cites — a separate link sweep caught systematic relative-path depth errors (3-deep concepts/ needs ../../../skills/, sources/ needs ../../core/); (c) profile.md = on-demand USER profile (session-start reads it), so project state went in state.md NOT profile.md. NEXT = Phase 4: 4 demonstrations (recover input traps) + cold-session smoke, then flip public
- autonomy: supervised
