...
- next: SHIP — real-data science comparison on a genuinely stellar field (drop-shape pixfrac<1 = 2nd-order) + workspace example/config exposing psf_backend="starred", THEN ship_library. Back-end functionally complete. Traps: reduction.json "96 stars"=JWST-config not strict point-src; worktree lacks gitignored outputs→reads canonical checkout; build_starred_epsf runs only in starred venv (astropy8/jax0.10≠PyAuto) w/ autoreduce on PYTHONPATH.
