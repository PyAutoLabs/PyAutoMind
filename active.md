...
- status: workspace-dev — STALE MARKER CONFIRMED. The parked NEEDS_FIX blamed a PyAutoGalaxy `__hash__` TypeError in `linear_light_profile_intensity_dict`; that bug was fixed by PyAutoGalaxy #374 (0dcea475, 2026-04-28) — one day AFTER the 2026-04-27 park — which also shipped the 5 regression tests the prompt asked for (test_autogalaxy/profiles/light/linear/test_abstract.py, 11 passed). Clean from-scratch reproduction of scripts/database/scrape/general.py (fresh Nautilus fit → scrape → aggregator → aplt.subplot_fit_imaging) EXITS 0, all three agg checks pass. NO PyAutoGalaxy change needed. Remaining work = retag no_run.yaml line 20 NEEDS_FIX → SLOW: measured 214s clean run vs the 60s cap (~45s viz warm-up + 83s Nautilus fit), and config/build/env_vars.yaml deliberately unsets PYAUTO_TEST_MODE/SMALL_DATASETS/DISABLE_JAX for `database/scrape/`, so it joins its four already-SLOW siblings (multi_analysis, slam_general, slam_multi_one_by_one, slam_pix). TRAP: autolens_workspace_test main has 25 pre-existing dirty binary dataset/** files — commit no_run.yaml BY EXPLICIT PATH only.
- worktree: ~/Code/PyAutoLabs-wt/scrape-general-stale-needs-fix
- autonomy: supervised
- prompt: active/linear_light_profile_hash_typeerror.md
- note: reclassified library+workspace → workspace-only once the library fix was found already shipped. One-line YAML retag.
- repos:
  - autolens_workspace_test: feature/scrape-general-stale-needs-fix
