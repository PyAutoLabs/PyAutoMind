...
- status: awaiting-merge — PR OPEN autolens_workspace_test#189 (pending-release), commit 194c1fd. Opened under corrective-PR exception for Heart RED reason "58 stale parked script(s)" (this PR removes 3); all other RED reasons pre-existing/unrelated. MERGE IS HUMAN. On merge: lifecycle.py record → complete. CONFIG bug: env_vars.yaml defaults set PYAUTO_DISABLE_JAX=1; jax_grad/imaging_{lp,mge,pixelization} overrides unset PYAUTO_SMALL_DATASETS but forgot PYAUTO_DISABLE_JAX → JAX-autodiff scripts take numpy xp path while value_and_grad traces → TracerArrayConversionError (lp, positions path) / all-zero grad (mge+pix, no positions path). NB current main=3d8d604: PR#186 (2026-07-21) added the THIRD marker (pixelization, misattributed to custom_jvp) — same DISABLE_JAX cause, CONFIRMED (pix fails variant-A "all zeros" under DISABLE_JAX=1, passes 7/7 variants with JAX on). PR#186 touched only yaml (scripts unchanged → repro valid). Fix = ONE folder-level jax_grad/ override unset [PYAUTO_SMALL_DATASETS, PYAUTO_DISABLE_JAX] (mirrors jax_likelihood_functions/; also wires point_source+weak, prev un-overridden+broken) + drop 3 stale no_run.yaml NEEDS_FIX lines. NO .py edits, NO library work. 2 files: config/build/{env_vars,no_run}.yaml. Validated: lp/mge/pix[7]/point_source/weak all pass AD=FD under fixed env; lp+pix reproduce original symptoms under DISABLE_JAX=1.
