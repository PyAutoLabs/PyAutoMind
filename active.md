...
- status: library-MERGED 2026-07-23 (human "merge all") — PyAutoArray#403 + citations PyAutoLens#644 + PyAutoGalaxy#520 all MERGED (403 needed a main-merge: version 2026.7.23.1 + LF restore, main's bump commit had CRLF'd autoarray/__init__.py). HowTo PRs HowToLens#53 + HowToGalaxy#43 MERGED on green CI. jax_grad migration RE-CERTIFIED (strict FD all params, all configs) + PR autolens_workspace_test#209 open, merging on green CI (claim override user-authorized 2026-07-23, only 2 jax_grad files, ENV lines untouched). REMAINING: (2) user-workspace pixelization example citation paragraphs — blocked same claim holder (autolens_workspace/autogalaxy_workspace); (3) developer-repo cleanup + Gut stash — blocked on blackjax-smc-gradient-kernel. On resume: Opus-delegated ship, regen notebooks+navigator for any HowTo/workspace edits.
