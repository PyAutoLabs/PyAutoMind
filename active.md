...
- session: claude --resume "unify-jax-visualization"
- status: issued
- classification: library
- suggested-branch: feature/unify-jax-visualization
- repos:
  - PyAutoFit: primary
  - PyAutoGalaxy: downstream
  - PyAutoLens: downstream
- summary: |
    Remove use_jax_for_visualization flag; have quick-update visualization
    reuse the search's JIT-compiled fit_from. Current path pays 20s of
    234 individual JAX JIT compiles on first quick update. Target: 0s
    (search already compiled it). Also investigate 5s steady-state
    cache-miss when using jax.jit(fit_from) directly.
