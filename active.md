...
- status: workspace-shipped, awaiting-merge — Phase 1a MERGED (PyAutoFit#1408) + Phase 1b PR autolens_workspace#311 OPEN (un-park interferometer Delaunay + add to smoke; local run_smoke 11/11). Phase 2 (PyAutoArray fnnls/mesh NaN producer) still open. [1a hist: PyAutoFit#1408 pending-release, RE-SCOPED PyAutoLens→PyAutoFit: full CI traceback showed failure runs via autofit _fit_bypass_test_mode abstract_search.py:872, not PyAutoLens analysis). Fix = TEST_MODE=2 bypass single-eval catches FitException → keep -1e99 sentinel + log; only FitException caught (real errors still fail). +2 tests, 108 search/result/samples pass. Heart RED at ship = all ambient/pre-existing (user ack'd). NEXT: Phase 1b (autolens_workspace, library-first AFTER #1408 merges) = un-park interferometer Delaunay no_run + add to smoke (via /start_workspace; workspace CI uses same-named PyAutoFit branch). Phase 2 (PyAutoArray, separate) = fix fnnls.py:134 divide-by-zero / degenerate Hilbert-mesh NaN producer. DIAGNOSIS: fails in source_pix_2 (Hilbert adaptive mesh, NOT Overlay — why 250 fit_from draws on Overlay stage were clean); TRUE flake (CI-thread-dependent, #307's 2 same-commit runs disagreed, no lib merge between).
- library-pr: https://github.com/PyAutoLabs/PyAutoFit/pull/1408
