...
- status: PHASE 0 SHIPPED, awaiting-merge (2026-07-17) — BIRTH UNBLOCKED. Two PRs open, MERGE ORDER MATTERS: (1) PyAutoBrain#137 (profile patterns + new check_boundary.py + 11 hermetic tests + docs), then (2) autolens_assistant#76 (maintainer.md prose + new .github/workflows/clone-boundary.yml). #76's `boundary` job FAILS UNTIL #137 MERGES — it runs #137's check_boundary.py which is not on main yet ("can't open file ... No such file or directory"); re-run #76's checks after #137 lands and it goes green. SHIPPED ON A HUMAN-AUTHORISED HEART-RED OVERRIDE (PR-open only, merge stays human): 5 RED reasons, NONE involving these repos (PyAutoBrain clean on main per Heart's own scan; autolens_assistant is not among the 25 repos Heart tracks) — dirty PyAutoLens checkout, workspace validation failed 2026-07-09, 58 stale parked scripts, manifest drift (tenant firewall, 7 mismatches), stale release validation. VERIFIED: 136/136 PyAutoBrain tests; clone analyze exit 0 / unclassified 0 (was exit 4 / 39); both refs clean (autolens 52·295·93·0, autofit 104·20·21·0, totals reconcile 440); --apply --mode lightweight-seed --no-push BUILT A SEED (145 copied, 388 PENDING, al_→ac_ correct); check_boundary proven to FAIL on an unclassified tracked file, SKIP on a non-reference, ERROR on a --reference typo. NEXT = Phase 1 (birth the seed) once both PRs merge
...
- library-pr: https://github.com/PyAutoLabs/PyAutoBrain/pull/137 (Phase 0, PyAutoBrain) + https://github.com/PyAutoLabs/autolens_assistant/pull/76 (Phase 0, reference cell) — merge #137 FIRST
- repos:
