...
- status: PHASE 2 MERGED (autocti_assistant#2, 557ba0b) — leg3 wiki-currency GREEN in CI (baseline held, no version drift); legs 1-3 ALL GREEN — PyAutoLabs/autocti_assistant PRIVATE. Ph0 merged (Brain#137+al_assistant#76), Ph1 born + seed-infra fixed (#1 merged). PHASE 2 = 9 ac_* skills grounded vs the 118 validated autocti_workspace scripts + verified against live stack: setup_environment (arcticpy), workspace_navigation, simulate_dataset_1d, simulate_imaging_ci, mask_and_extract, fit_cti_model (factor-graph joint fit = core), correct_cti, plot (function API), load_results (CTIAgg). Wired: .claude/skills symlinks + REWRITTEN honest skills/README (old = autolens catalogue, 45 listed/13 existed) + PENDING pruned. VALIDATION all green: --scope all (0 broken/33 sym), --check-citations (0 missing/27), --lint-idioms, --check-provenance, --check-version (WROTE wiki/core/api_audit_baseline.json pinning 4-module CTI stack, NO autogalaxy). GROUNDING CORRECTED PROMPT: aggregator is ac.agg.CTIAgg not Dataset1DAgg; plot scripts at scripts/plot/ not per-geometry. AUDIT TRAP: writing a defunct symbol/idiom in PROSE to say it's gone (aplt.Output, 'analysis + analysis', wildcard aplt.subplot_*) trips the symbol/idiom checker — rephrase so no live-looking token. PUBLISH GATE legs 1-3 ALL GREEN (CI confirmed); leg 4 = Phase 4. NEWBORN STAYS PRIVATE until Ph3 wiki + Ph4 demos done. NEXT: Ph3 = wiki/core (CTI physics, trap model, FPR/EPER, arctic algo, parallel vs serial) + wiki/literature (public sources, verified cites) + profile.md; Ph4 = 4 demonstrations (recover input traps) + cold-session smoke, then flip public
- autonomy: supervised
