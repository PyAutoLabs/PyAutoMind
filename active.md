...
- status: steps 4+5+6(stages 1+2) MERGED 2026-07-23 — all 14 rename PRs in (see #181 comment for list). Post-merge on live mains: branch+main smoke CI green ×10 repos; strict validator 0/0 ×3 _test repos; byte-identical env resolution over 288 scripts; sweep clean. REMAINING: stage 3 = drop legacy env_vars*.yaml fallbacks (autohands PROFILE_NAMES, 10 vendored run_smoke.py, Heart workflows) + validator ERROR on legacy names — small per-repo PRs, any time. Step 7 (one-reader fold) blocked on testmode-env-drift PyAutoFit claim. Step 8 = ASK HUMAN first, may be declined. Worktree has 14 repos attached — clean up merged branches when task closes.
