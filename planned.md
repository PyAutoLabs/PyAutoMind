...
- prompt: PyAutoMind/issued/adopt_the_lenstool_reference_magnitude_mag0_conv.md
- issue: https://github.com/PyAutoLabs/autolens_workspace/issues/265
- status: blocked
- filed: 2026-07-10
- classification: workspace (autolens_workspace) — docs, LensTool scaling-relation convention
- suggested-branch: feature/lenstool-scaling-reference-magnitude
- blocked-by: markdown-renderings-workspaces (batch 2a, #264) holds the autolens_workspace worktree — active --auto build; memory warns "don't edit the workspace tree mid-build"
- summary: |
    Adopt LensTool reference-magnitude (mag0) convention for the scaling-relation
    tier across autolens_workspace examples: explicit fixed reference luminosity
    (not max-of-sample), fixed exponent 0.5, full dPIE r_core/r_cut/b0 scaling
    (add ra_ref). Scope (grep + adversarial-verified): 3 feature families
    (cluster, group/scaling_relation, imaging/scaling_relation) + 2 SLaM
    pipelines (group/slam.py, group/features/linear_light_profiles/slam.py; 4
    sites) + simulators + regenerated notebooks. Ship as 3 sequential PRs.
    File sets are nearly disjoint from batch 2a (that edits config/*.yaml, cluster
    EXCLUDED; ours edits scripts/) — unblock when 2a merges OR user overrides to
    run a concurrent worktree on a separate branch.
- affected-repos:
