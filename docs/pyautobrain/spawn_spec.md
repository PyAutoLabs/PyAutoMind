...
| 2 | `REFERENCE.md`, `AGENTS.md`, `CLAUDE.md`, `LICENSE`, `ROUTING.md`, `.gitignore`, `.gitattributes` | KEEP verbatim (`.gitattributes` is the `merge=union` driver for `autonomy_log.md`, which rule 5b ships — the attribute travels with the file it governs) |
...
| 6 | `draft/**` (the work-type dirs `feature/ bug/ refactor/ docs/ test/ release/ maintenance/ research/ triage/` now live under `draft/`) | SKELETON → keep a single `draft/.gitkeep`; drop all draft prompts and their work-type/target subdirs (a fresh Mind starts with an empty `draft/`; intake recreates the work-type subdirs on demand) |
...
| 7d | `state.json` | DROP — the organ-cockpit feed (PyAutoBrain `board/_state.py` v1), written by the same `pyauto-brain intake --apply dashboard` run and published by the same `pages_dashboard.yml` (dropped by rule 9c). Same reasoning as 7c: nothing shipped reads it until a fresh org regenerates the dashboard |
...
The smoke-bootstrap propagator also follows rule 9c: its cross-repository pushes
require an organization token and opted-in sibling repositories. Its generic
generator, canonical block and tests travel; the token-dependent workflow does
not. A new organism adopts delivery deliberately after configuring its manifest.
...
| 3 | `wiki/AGENTS.md` (the canonical shared schema) and `wiki/CLAUDE.md` (thin adapter) | KEEP verbatim — since the wiki/ restructure (PyAutoMemory#24) the schema is a single domain-neutral file at `wiki/AGENTS.md` with a thin `wiki/CLAUDE.md` adapter, kept canary-clean at the source (its examples carry no instance tokens), so spawn no longer maintains a duplicate schema asset |
| 4 | `wiki/*` (all live sub-wikis, all pages) | DROP; generate instead ONE `wiki/example/` containing a slim scope-only `AGENTS.md` and thin `CLAUDE.md` adapter (the schema is inherited from rule 3, not copied), an `index.md` skeleton listing zero sources, and one `sources/EXAMPLE_stub.md` demonstrating the stub format (hand-written once, stored inside spawn as a heredoc/template asset — not copied from live content) |
