...
**Epic COMPLETE 2026-09-06.** All nine phases shipped (1–4 the board, 7 the caches, 5–6 the `_test` rebuilds, 8 + 8c the user surface and its library leg, 9 the census). Left as ordinary backlog: phase 8b `draft/test/autocti_workspace/imaging_ci_start_here_61s.md` (retired 2026-09-14 into `complete/2026/09/ci-smoke-speedup.md`); the four phase-6 follow-ups (`draft/bug/autolens_workspace_test/*`, `draft/bug/autoarray/{mapper_adapt_zero_signal_jax_nan,jax_delaunay_returns_more_than_five_values,mixed_precision_inversion_jax_numpy_gap_small_data}.md`); the post-release loose ends; and whichever census options the human accepts. Three library releases (PyAutoNerves, PyAutoArray, PyAutoFit) are outstanding.
...
| 8 | complete/2026/09/user-workspace-howto-slow-script-pass.md | **SHIPPED** 2026-09-06 — HowToLens#77 (`3d60e73`) + HowToGalaxy#73 (`92e1a75`) merged, autolens_workspace#536 closed; 14-script diagnosis: 5 import floor, 4 re-simulation, 3 uncapped meshes (fixed: 14.0/10.0/7.7 → 4.5/4.2/4.1 s; 11.9 → 4.6 s), 1 `replace_promise`, 1 genuine full-res; the local-vs-CI gap was a cold numba cache; library findings → phase 8c; 8b `draft/test/autocti_workspace/imaging_ci_start_here_61s.md` (retired 2026-09-14 → complete/2026/09/ci-smoke-speedup.md) |
| 8c | complete/2026/09/{replace-promise-no-op-graph-walk,small-datasets-cap-stamp-stops-resimulation,sparse-operator-ignores-disable-jax,mesh-shape-honours-small-datasets-cap}.md | **SHIPPED 2026-09-06** — PyAutoNerves#160 (`efe7c04`) → PyAutoArray#529 (`bcd15cd9`), PyAutoFit#1564 (`cab17a3ee`) merged, issues #159/#528/#1563 closed; re-simulation gone on the capped interferometer/multi datasets (~26–28 s per user-workspace CI run), `model.info` 10.6 → 6.7 s, mesh shapes cap with the data; three library releases outstanding (`pending-release`); post-release loose ends in `draft/maintenance/autoarray/small_datasets_followups_after_8c.md` |
| 9 | complete/2026/09/unit-test-import-time-hotspot-census.md | **SHIPPED 2026-09-06** — PyAutoHeart#214 (`20eea5b`) merged, issue #213 closed; the census `timings/unit_import_census_2026-09.md`: library CI is compile-bound (`lib-tests.yml` restores no JAX/numba cache), `--cov` is a third of the legs, one kaplinghat test is 52 % of PyAutoGalaxy, three imports paid and not needed, PyAutoFit unparallelisable, the NUFFT "numpy" path is JAX; nine ranked options, O1 = caches for `lib-tests.yml`; each accepted option becomes its own prompt |
...
## Retired from epics.md (2026-09-06)
...
- status: COMPLETE 2026-09-06 — all nine phases shipped in two days (board: PyAutoHeart#203/#205/#207/#209; caches: #211; `_test` rebuilds: autogalaxy_workspace_test#117, autolens_workspace_test#294, epoch #212; user surface: HowToLens#77 + HowToGalaxy#73; library leg: PyAutoNerves#160, PyAutoArray#529, PyAutoFit#1564; census: PyAutoHeart#214). Gates: autogalaxy_test 515 → 438 s, autolens_test 396 → 322 s cold locally, CI legs 8.5 → 5.5 min warm; the record (`timings/`), the `legacy`/`fast-tests` epochs and the census are the instrument the next round starts from. Backlog left: phase 8b (autocti), the phase-6 follow-ups, the post-release loose ends, the census options the human accepts; three library releases outstanding.
