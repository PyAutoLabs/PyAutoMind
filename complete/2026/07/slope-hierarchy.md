...
- library-pr: https://github.com/PyAutoLabs/PyAutoFit/pull/1383 (merged — the projection fix this project's goal-4 diagnostics CAUGHT)
- workspace-pr: none (external science project on its own main at /mnt/c/Users/Jammy/Science/slope_hierarchy — no PyAutoLabs repo claimed; ic50_workspace-style non-standard)
- notes: ALL FOUR GOALS ANSWERED; wrapped up with the tree clean and everything pushed. Hierarchical power-law slope recovery from N=5 simulated imaging lenses: per-lens `slope` drawn from a parent Gaussian N(2.0, 0.1), fit two ways on the SAME factor graph. GOAL 1 (BlackJAX-NUTS joint fit) SUCCESS: parent mean 2.028 [2.000, 2.063], scatter 0.143 [0.117, 0.185] vs truth (2.0, 0.1) — NUTS is the trustworthy method. GOAL 3 (RAL HPC pipeline) works end to end. GOAL 4 (exercise the 2026-07 EP diagnostics wave, PyAutoFit#1330/#1335) is the standout: the diagnostics caught a REAL projection bug in the framework running them — TWO stacked defects, TransformedMessage.project moment-matching PHYSICAL samples instead of mapping through the transform into base space (pinning messages at prior bounds), and Result.projected_model feeding LINEAR weights to a log-weight projection — shipped as PyAutoFit#1383 (merged). GOAL 2 (EP vs NUTS parity in values AND errors) = characterised PARTIAL parity, the honest negative result: converged EP recovers the parent MEAN (2.051, agrees with NUTS) but its converged SCATTER is ~4x low (0.026 vs truth 0.1 / NUTS 0.143) with errors ~1000x too tight. Getting there took three separations, each of which could have been mistaken for the answer: (a) the projection bug (real, fixed, #1383); (b) UNDER-CONVERGENCE — the round-1 max_steps=5 value of 0.004 was a badly under-converged snapshot, not a result; job 330639 at max_steps=12 climbed 0.0007 -> 0.0037 -> 0.0135 -> 0.0264 and plateaued flat for the last ~2 sweeps, so 0.026 is the true EP fixed point; (c) problem-specific vs framework — settled by spinning out the cheap CPU toy (see the ep-hierarchical-scale-collapse record, same day): FRAMEWORK, and specifically stochastic instability (70% recover / 7% collapse / 23% crash), reported as PyAutoFit#1405. TRAP: naive damping made it WORSE (delta=0.5 -> full collapse, 67 BAD_PROJECTION, log-evidence 5e7), so the diagnostics' "consider damping" hint mis-diagnoses this mode. The standing methods claim: use a joint gradient sampler for a hierarchical SCALE hyperparameter; EP is fast and correct for the parent MEAN but over-confident and variance-shrinking on the scatter — and because the failure is a ~7% basin rather than a fixed bias, one EP run that recovers the truth disproves nothing. Repo moved to autonerves mid-project (autoconf rename, PRs #2/#3, 2026-07-18). PyAutoFit was exercised NOT edited except via #1383. REMAINING WORK WAS OPTIONAL AND IS NOW FILED, not dropped: draft/research/graphical_ep/slope_hierarchy_n25_scale_up.md (N=25-50 scale-up; carries the RESUME traps — rm output/<sample>/* before refits, force-sync truth files, verify the RAL PyAutoFit mirror commit, export JAX_ENABLE_X64=True explicitly in sbatch) and draft/research/graphical_ep/slope_hierarchy_methods_writeup.md (Opus prose; paper/figures and paper/tables are still EMPTY; scope decision — methods note now vs publishable paper which wants the scale-up first). Issue #1 left OPEN by design as the project's umbrella.
