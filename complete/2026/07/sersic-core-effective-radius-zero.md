...
- workspace-pr: https://github.com/PyAutoLabs/autolens_workspace/pull/306
- summary: SersicCore.intensity_prime scalar `/ self.effective_radius` → xp.divide, so effective_radius<=0 yields inf (search resamples) not ZeroDivisionError — matching every other profile (Sersic uses numpy-array division) + the JAX path (numpy-vs-jax raise-vs-resample, PyAutoLens#607 lineage). Root cause was NOT alpha (alpha=3.0 Constant, probe-verified); crash was effective_radius=0 from the multi-wavelength wavelength*m+c relation's symmetric-about-0 m,c priors giving 0 at the test-mode median. PyAutoGalaxy full suite 1004 passed + new numpy unit test (effective_radius=0 → non-finite, no raise). Workspace had a SECOND independent gap: multi/features/wavelength_dependence/modeling.py lacked the auto-simulate block its sibling multi/modeling.py has (FileNotFoundError in a fresh worktree) — added it; with both fixes the script runs green from clean, no_run marker dropped. HowToLens had no such tutorial (dead path, no change). Split out from #300.
