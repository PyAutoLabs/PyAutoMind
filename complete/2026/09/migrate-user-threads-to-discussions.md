The historical Discussions backfill was deliberately retired on 2026-09-21.
The maintainer decided that moving old, already-shipped user feature-request
issues into the new org Discussions hub is not important enough to justify the
manual GitHub-UI work.

## What was completed

- The org-wide Discussions hub is live and the user-facing routing is already
  established.
- PyAutoArray, PyAutoLens and PyAutoGalaxy READMEs and issue choosers point
  users to the org hub, while repository Discussions remain off.
- The reproducible `imshow_origin` report preserved as org Discussion #14
  was checked against current PyAutoArray `main`; the defect is still present.
- PyAutoArray#565 was opened as the linked development tracker and labelled
  `bug`.

## Deliberately waived

The remaining historical feature-request conversions are not required:
PyAutoArray#499, PyAutoLens#631/#564/#542 and PyAutoGalaxy#419 may remain as
issues. No future dashboard or maintenance task should treat their migration as
outstanding work.

Discussion #14 may still be moved to **Bugs & Errors** and marked settled in
the GitHub UI if convenient, but that presentation cleanup is explicitly not a
blocker.

## Doctrine update

`policy/community_surface.md` now records that historical backfill is optional
and that the important contract is prospective routing: community questions,
ideas and proposals go to the hub; confirmed/reproducible development work gets
a linked target-repository issue.
