# Point-source CPU speed-up campaign — phase 2: throwaway vertex-dedup removal

Removed the JAX-only throwaway `jnp.unique` vertex dedup on the PointSolver trace path and measured it with an interleaved in-process A/B on RAL CPU and a RAL A100. Phase 2 of the phased CPU campaign only; the prompt's phases 3–4 remain open and are re-filed as `draft/research/autolens_profiling/pointsolver_cpu_speed_phases_3_4.md`, pointing at this record. Issue PyAutoArray#568 stays open for them.
...
- Merged [PyAutoArray#569](https://github.com/PyAutoLabs/PyAutoArray/pull/569) at `681938ae616f47e1fc4b3ef7812fb6c33c0d0443` (branch head `25894d10`: fix `273e152e` + tests), then [autolens_profiling#301](https://github.com/PyAutoLabs/autolens_profiling/pull/301) at `93757902b512120c9ee17eaa6334a90eb6bcceb5` (branch head `680c0b77`) — library first, human `/prm` 2026-09-24, all CI legs green (PyAutoArray unittest 3/3, profiling lint).
- PyAutoArray: `CoordinateArrayTriangles._vertices_and_indices` on the JAX path returns the flat vertex table plus an identity index map instead of `jnp.unique(..., size=3N)` (which under `jit` saved zero deflections and cost one fp64 lexicographic sort per refinement step). API unchanged; NumPy sibling untouched. New `test_autoarray/structures/triangles/test_coordinate_jax.py` (10 tests, incl. an HLO no-sort guard that is red on main). PyAutoLens needed no change.
- autolens_profiling: `scripts/point_source/likelihood_breakdown/vertex_dedup_ab.py` (control / nodedup / library routes, distinct function objects + `jax.clear_caches()`, import-time source + bytecode-hash provenance), RAL CPU and A100 submits, results `results/breakdown/point_source/vertex_dedup_ab_{hpc_ral_cpu,hpc_ral_a100,local_cpu}_fp64.{json,png}`, job logs, campaign ledger `results/notes/point_source_cpu_campaign.md` phase 2 (DONE, ACCEPTED).

## Evidence

- RAL CPU job 350582 (`ral`, 8 CPUs, EPYC 7763, fp64): simple solved 24.00 → 5.38 ms (**4.47×**, 90 % CI 4.37–4.57, MDI 4.8 %); simple vmap-4 2.63×; two-source cluster solved 155.22 → 78.32 ms (**1.98×**, CI 1.93–2.03, MDI 13 %). HLO sorts 15 → 7 (simple) / 25 → 12 (cluster); FLOPs 15.58M → 7.05M / 184.2M → 139.4M; compile −20 % / −11 %.
- RAL A100 job 350587 (fp64, 20 rounds × 20 calls): no GPU regression — nodedup 1.71–2.05× faster than control; the imported library (`25894d10`) is hash-identical to nodedup.
- Correctness: all 19 gates pass on both hosts — log-likelihood bit-identical on 16 stream instances (fiducial `7.743201200876812`), solved positions `array_equal`, `jax.grad` bit-identical, vmap-4 equals scalar; PyAutoArray suite 1626 passed; autolens_workspace_test point-source `jax_likelihood` ×4 + `jax_grad` identical to main; profiling pin unchanged.
- September hypotheses (4.9× simple / 2.4× cluster) reproduce in direction and order of magnitude; RAL's 4.47× / 1.98× supersede them. Cluster control is 15–20 % above phase 1 because phase 1 ran on a Xeon 8490H node and phase 2 on EPYC 7763 — host difference, not drift.
- Post-fix budget: deflections are **70 %** of the simple call's FLOPs → phase 3 (precompute the static initial lattice) ranked first. Duplicate `model_data` solve confirmed CSE'd (0.34 % FLOP gap) — closed.
...
- The merge itself was the human's `/prm` on 2026-09-24; Heart remains RED for release purposes.

## Handoff

- Remainder (phases 3–4) re-filed: `draft/research/autolens_profiling/pointsolver_cpu_speed_phases_3_4.md`; issue PyAutoArray#568 left open for it.
- First step after the PyAutoArray release: `HPCPullPyAuto` on RAL, re-run the phase-1 `point_source` and `cluster` `likelihood_breakdown/image_plane.py` cells under a new label so the README dashboard rows move (record node CPU model), re-run `vertex_dedup_ab.py` (library route must read `library_matches: nodedup`), then phase 3.
- Follow-ups found, not fixed: `jax.grad` of `AnalysisPoint` silently all-zero without `autofit.jax.register_model(model)` (library issue candidate, PyAutoFit/PyAutoLens — raise or warn); CPU A/B ran 5 calls/round, not the planned ≥ 20; cell does not record XLA intra-op threads or per-route memory; `ral` host pinning (Xeon 8490H vs EPYC 7763 — use `--nodelist`/`--constraint` or an in-job control); RAL cleanup of the `/mnt/ral/jnightin/autolens_profiling_wt/PyAutoArray_point-source-cpu-p2` clone and the `_p2_untracked_backup_20260924` dir; still open from phase 1: `check_submits.py` `python3 -u` regex gap, `activate.sh` worktree-leak guard, CI smoke coverage for the breakdown cells (now incl. `vertex_dedup_ab.py`).
