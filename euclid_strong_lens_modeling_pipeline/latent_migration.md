...
1. **Delete the custom `LATENT_KEYS` + `compute_latent_variables`** from `util.py:306-490`. Keep any helper functions that are still useful (e.g. the magzero/muJy converters) only if they weren't ported into PyAutoGalaxy in sub-prompt #1. If they were ported, delete from util.py.
2. **Simplify the `AnalysisImaging` subclass** in util.py to keep only what is genuinely Euclid-specific (e.g. the custom `VisualizerImaging` setting the RGB subplot). The latent machinery is gone.
