...
- status: phase 1 **SHIPPED** 2026-09-05 — PyAutoHeart#203 (`7a286ea`) merged, issue #202 closed, record `complete/2026/09/smoke-timings-ingester.md`; watch the first 05:00 UTC heart-health run for `N unavailable` (cross-repo artifact 403 → add the `HEART_TIMINGS_TOKEN` secret); Fable review of the plan recorded in the ledger (recommends 1→2→3→4→7→5→6→8→9 — CI caches before the rebuild waves); next: issue phase 2 `draft/feature/pyautoheart/permanent_ci_timing_history.md`
