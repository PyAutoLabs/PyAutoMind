...
- status: phase 1 **SHIPPED** 2026-09-05 — PyAutoHeart#203 (`7a286ea`) merged, issue #202 closed, record `complete/2026/09/smoke-timings-ingester.md`; watch the first 05:00 UTC heart-health run for `N unavailable` (cross-repo artifact 403 → add the `HEART_TIMINGS_TOKEN` secret); Fable review of the plan recorded in the ledger (recommends 1→2→3→4→7→5→6→8→9 — CI caches before the rebuild waves); phase 2 **SHIPPED** 2026-09-05 — PyAutoHeart#205 (`7b4e109`) merged, issue #204 closed, record `complete/2026/09/permanent-ci-timing-history.md` (the record's data files appear with the first daily run); phase 3 **SHIPPED** 2026-09-05 — PyAutoHeart#207 (`cb1d500`) merged, issue #206 closed, record `complete/2026/09/offtick-timing-legs-live.md`; phase 4 **SHIPPED** 2026-09-06 — PyAutoHeart#209 (`ad56720`) merged, issue #208 closed, record `complete/2026/09/legacy-baseline-timing-round.md` (epoch `legacy @ 2026-09-05` live; digest at PyAutoHeart `timings/legacy_round_2026-09.md`; the `HEART_TIMINGS_TOKEN` fallback was not needed); phase 7 **SHIPPED** 2026-09-06 — PyAutoHeart#211 (`bfcd302`) merged, issue #210 closed, record `complete/2026/09/smoke-ci-caches-jax-datasets.md` (JAX compile + dataset caches in the reusable smoke workflow; cache state on every timing row; hot-vs-cold numbers pending two live runs per repo); phase 5 **SHIPPED** 2026-09-06 — autogalaxy_workspace_test#117 (`0f69588`) merged, issue #116 closed, record `complete/2026/09/physical-fast-rebuild-autogalaxy.md` (39/39 green, 515 s → 438 s cold locally, −15%; CI legs green on cold caches; mesh-vs-masked-pixel rule and the mixed-precision follow-up are phase 6's inputs; the ~230 s import floor is phase 8/9's, compile is the CI cache's); phase 6 **SHIPPED** 2026-09-06 — autolens_workspace_test#294 (`fd4e17f`) merged, issue #293 closed, record `complete/2026/09/physical-fast-rebuild-autolens.md`; `fast-tests` epoch PyAutoHeart#212 (`6d5c1c4`) — the rebuild waves are done (27/27 green, 396 s → 322 s cold, CI 8.5 → 5.5 min warm per leg; four follow-ups filed); phase 8 **SHIPPED** 2026-09-06 — HowToLens#77 (`3d60e73`) + HowToGalaxy#73 (`92e1a75`) merged, autolens_workspace#536 closed, record `complete/2026/09/user-workspace-howto-slow-script-pass.md` (the user surface is import-floor bound; the remaining wins are library-side); phase 8c **PRs OPEN** 2026-09-06 — PyAutoNerves#160 → PyAutoArray#529 (merge in that order), PyAutoFit#1564 (issues #159/#528/#1563; re-simulation gone, `model.info` 10.6 → 6.7 s, both `_test` gates green with the branches installed); phase 9 **PR OPEN** 2026-09-06 — PyAutoHeart#213 → #214 awaiting merge (`active/unit_test_import_time_hotspot_census.md`; the census: library CI is compile-bound — `lib-tests.yml` restores no cache — plus eight more findings and nine ranked options; the epic closes when #214 and the 8c PRs merge); autocti's 61 s outlier is phase 8b
